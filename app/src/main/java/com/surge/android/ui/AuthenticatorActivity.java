...
//    private UserLoginTask mAuthTask = null;
        ...
        if(intent.getStringExtra(PARAM_EMAIL) != null) {
            ...
            for (Account account : accounts) {
                ...
                if (type.equals("com.google")) {
                    mSignInUserView.setText(possibleEmail);
                    mSignUpEmailView.setText(possibleEmail);
                    break;
                }
            }
    ...
    public void onResponse(AuthenticateResponse response) {
        ...
        mMixpanel.alias(response.getUser().getUserId(), null);
        people.set("$surge_user_id", response.getUser().getUserId());
        people.set("$username", response.getUser().getUsername());
        people.set("$email", response.getUser().getEmail());
        people.set("$surge_points", response.getUser().getSurgePoints());
        showProgress(false);
    ...
    private void finishConfirmCredentials(boolean result) {
        ...
        final Account account = new Account(mSignInUserView.getText().toString(), getString(R.string.account_type));
    ...
    private void finishLogin(AuthenticateResponse response) {
        ...
        final Account account = new Account(response.getUser().getUsername(), getString(R.string.account_type));
        if (mRequestNewAccount) {
            Log.v(TAG, "New account");
            mAccountManager.addAccountExplicitly(account, password, null);
            mAccountManager.setAuthToken(account, getString(R.string.token_type), response.getToken());
            //TODO(gil): this?
